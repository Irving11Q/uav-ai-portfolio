...
def build_agent(model, tools=None, checkpointer=None):
    ...
    Args:
        model: 绑定好工具的模型
        tools: 工具列表。不传就用本文件的两个电池工具。
               ⭐ 传别的列表 = 换一套工具，图一行都不用改 ——
                  Day22-A 就是这么把「电池体检 Agent」变成「手册问答 Agent」的。
                  记住这句话：图是通用骨架，工具是插拔件。
    ...
    tool_list = list(tools) if tools is not None else list(TOOLS)
    # 【解释】默认用本文件的工具；传进来就用传进来的。
    #         这个参数是 Day22-A 踩坑后才加的 —— 原先写死 ToolNode(TOOLS)，
    #         导致换了工具的模型调了一个执行节点里根本不存在的工具。
    ...
    #         ⚠️ 这里的工具必须和 model.bind_tools(...) 绑的是同一套，
    #            否则模型点名要调某个工具、执行节点却不认得，只能回一句
    #            「xxx is not a valid tool」，然后模型被迫凭常识瞎答。
    #            （Day22-A 实测踩到过 —— 这是个很容易忽略的隐蔽 bug。）
